...
const DashboardForm2 = ({ nextStage, prevStage, setCanProceed }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "/api/v1/users/Noc-Certified-Copy",
        submitData,
        config
      );
      ...
      if (response.data.success) {
        ...
        setUserId(newId);
        setSuccessMessage(
          "Form submitted successfully! You can now proceed to the next step."
        );
        setErrorMessage("");
        setCanProceed(true);
    ...
    <div>
      <div className="min-h-screen bg-gray-50 px-0 py-0 flex justify-center items-center">
        <div className="w-full max-w-6xl bg-white rounded-lg shadow p-6">
          ...
          {/* Form */}
          <form
            className={`mt-6 text-sm ${window.innerWidth < 768 ? 'flex flex-col gap-4' : 'grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4'}`}
          ...
          >
            ...
            <div className="flex-1">
              <label className="font-medium block mb-1">
                गाव <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                name="village"
