...
const DashBoardForm9 = forwardRef(
  ({ nextStage, prevStage, setCanProceed, onFormSubmit }, ref) => {
    ...
    const [governmentServiceBranches, setGovernmentServiceBranches] = useState(
      []
    );
    ...
    useEffect(() => {
      const fetchGovernmentServiceBranches = async () => {
        setIsLoadingBranches(true);
        setBranchError("");
        try {
          const response = await axios.get(
            "/api/v1/government-service-branches"
          );
          if (response.data && response.data.data) {
            setGovernmentServiceBranches(response.data.data);
          } else {
            setBranchError("Failed to load government service branches");
          }
        } catch (error) {
          console.error("Error fetching government service branches:", error);
          setBranchError(
            "Error loading government service branches. Please try again later."
          );
        } finally {
      ...
      fetchGovernmentServiceBranches();
    }, []);
            ...
            >
              ...
              <div className="flex-1">
                ...
                {isLoadingBranches ? (
                  ...
                  <div className="w-full border rounded px-3 py-2 bg-red-50 text-red-600">
                    {branchError}
                  </div>
                ) : (
                  <select
                    ...
                    required
                  >
                    <option value="">Select Government Service Branch</option>
                    {governmentServiceBranches.map((branch) => (
