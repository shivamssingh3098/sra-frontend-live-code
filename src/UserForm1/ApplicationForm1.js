...
import DashBoardForm1 from "./DashBoardForm1";
        ...
        <div className="mt-8">
          ...
          <div className="space-y-4">
            <p>
              ...
              {formData.village || "_______"} मधील गाळा क्र.{" "}
...
const ApplicationForm1 = () => {
  ...
  // Add useEffect to check for existing form data
  useEffect(() => {
  ...
  const handleFormSubmit = (responseData) => {
    ...
    if (responseData && responseData._id) {
      ...
      let bhukhandNo = "";
      if (responseData.surveyNo) {
        bhukhandNo = responseData.surveyNo;
      } else if (responseData.finalPlot) {
        bhukhandNo = responseData.finalPlot;
      } else if (responseData.landNumber) {
        bhukhandNo = responseData.landNumber;
      }

      // Determine which ward type was selected
      let wardNo = "";
      if (responseData.sectorNo) {
        wardNo = responseData.sectorNo;
      } else if (responseData.wardNo) {
        wardNo = responseData.wardNo;
      }

      // Determine which council type was selected
      let nagarpalika = "";
      if (
        responseData.villageCouncil &&
        responseData.villageCouncil !== "NONE"
      ) {
        nagarpalika = responseData.villageCouncil;
      } else if (
        responseData.municipalCorporation &&
        responseData.municipalCorporation !== "NONE"
      ) {
        nagarpalika = responseData.municipalCorporation;
      } else if (
        responseData.cityCouncil &&
        responseData.cityCouncil !== "NONE"
      ) {
        nagarpalika = responseData.cityCouncil;
      }
      ...
      const updatedFormData = {
        ...
        date: responseData.applyDate || formData.date,
        address: responseData.address || formData.address,
        mobile: responseData.mobie || formData.mobile,
        taluka: responseData.taluka || formData.taluka,
        bhukhandNo: bhukhandNo || formData.bhukhandNo,
        village: responseData.village || formData.village,
        district: responseData.city || formData.district,
        wardNo: wardNo || formData.wardNo,
        nagarpalika: nagarpalika || formData.nagarpalika,
        schemeDeveloper:
          responseData.schemeDeveloper || formData.schemeDeveloper,
        governmentServiceBranch:
          responseData.governmentServiceBranch ||
          formData.governmentServiceBranch,
      };
  ...
  const nextStage = () => {
    if (currentStage === 0 && !canProceed) {
      // If on first stage and can't proceed, show error
  ...
  return (
    <div className="min-h-screen p-4 bg-gray-100 flex justify-center">
      <div className="w-full max-w-4xl bg-white p-6 rounded shadow-md">
        ...
        {currentStage === 1 && formData._id && (
          <UploadForm1
            formData={formData}
            formId={formData._id}
            dashboardData={formData} // Explicitly pass dashboard data
          />
        )}
        {currentStage === 2 && formData._id && (
          <Form1
            formData={formData}
            formId={formData._id}
            dashboardData={formData} // Explicitly pass dashboard data
